...
const extendedApi = omdbApi.injectEndpoints({
  endpoints: (build) => ({
    ...
    >({
      query: ({ mediaType, genreId, page }) => ({
        ...
        params: { api_key: OMDB_API_KEY, with_genres: genreId, page },
      }),
      transformResponse: (
    ...
    >({
      query: ({ mediaType, apiString, page }) => ({
        ...
        params: { api_key: OMDB_API_KEY, page },
      }),
      transformResponse: (
        response: PaginatedMovieResult,
        _,
    ...
    >({
      query: ({ mediaType, id }) => ({
        url: `/${mediaType}/${id}`,
        params: { api_key: OMDB_API_KEY, append_to_response: "videos" },
      }),
    }),
